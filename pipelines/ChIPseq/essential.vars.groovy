...
ESSENTIAL_MACS2_GSIZE=""  // mapable genome size for MACS2 (approx. size in bp or use "hs" for human, "mm" for mouse, "ce" for worm, "dm" for fly)
...
// Note that DiffBind works with "default" parameters which depend on the context of other parameter settings (see DiffBind documentation for explanation).
// In "contrasts_diffbind.txt" you can specify sub-experiments defining which sample groups to be combined in a model (otherwise DiffBind will include all samples in a single model).
RUN_DIFFBIND=true
