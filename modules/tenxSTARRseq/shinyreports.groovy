shinyReports = {
    ...
    produce("shinyReports.txt") {
        exec """
            ...
            cp ${PIPELINE_ROOT}/tools/reports/shiny_tenxstarrseq_reporting_tool/server.R ${REPORTS}                &&
            cp ${PIPELINE_ROOT}/tools/reports/shiny_tenxstarrseq_reporting_tool/ui.R ${REPORTS}                    &&
            cp ${PIPELINE_ROOT}/tools/reports/shiny_tenxstarrseq_reporting_tool/sc.shinyrep.helpers.R ${REPORTS}   &&
            cp ${PIPELINE_ROOT}/tools/reports/shiny_tenxstarrseq_reporting_tool/bustard.pl ${REPORTS}              &&
            cp ${PIPELINE_ROOT}/tools/reports/shiny_tenxstarrseq_reporting_tool/BustardSummary.toMD.xsl ${REPORTS} &&
            cp ${PIPELINE_ROOT}/tools/reports/shiny_tenxstarrseq_reporting_tool/styles.css ${REPORTS}              &&
            cp ${PIPELINE_ROOT}/tools/reports/shiny_tenxstarrseq_reporting_tool/app.R ${REPORTS}                   &&

            if [ -e "${REPORTS}/tenxstarrseq.report.Rmd" ]; then
                echo "tenxstarrseq.report.Rmd already exists. Older copy will be kept and not overwritten";
            else
                cp ${PIPELINE_ROOT}/tools/reports/shiny_tenxstarrseq_reporting_tool/tenxstarrseq.report.Rmd ${REPORTS};
            fi &&
            ...
            echo "SHINYREPS_SCE=${shinyReports_vars.sce}"         >> $output &&
