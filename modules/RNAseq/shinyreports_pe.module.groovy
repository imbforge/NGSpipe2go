...
shinyReports = {
    ...
    produce("shinyReports.txt") {
        exec """
            cp ${MODULE_FOLDER}/../tools/reports/shiny_rnaseq_pe_reporting_tool/server.R ${REPORTS}                &&
            cp ${MODULE_FOLDER}/../tools/reports/shiny_rnaseq_pe_reporting_tool/ui.R ${REPORTS}                    &&
            cp ${MODULE_FOLDER}/../tools/reports/shiny_rnaseq_pe_reporting_tool/DE.shinyrep.helpers_pe.R ${REPORTS}   &&
            cp ${MODULE_FOLDER}/../tools/reports/shiny_rnaseq_pe_reporting_tool/bustard.pl ${REPORTS}              &&
            cp ${MODULE_FOLDER}/../tools/reports/shiny_rnaseq_pe_reporting_tool/BustardSummary.toMD.xsl ${REPORTS} &&
            cp ${MODULE_FOLDER}/../tools/reports/shiny_rnaseq_pe_reporting_tool/styles.css ${REPORTS}              &&
            
            if [ -e "${REPORTS}/DEreport_pe.Rmd" ]; then
                echo 'DEreport.Rmd already exists. Older copy will be kept and not overwritten';
            else
                cp ${MODULE_FOLDER}/../tools/reports/shiny_rnaseq_pe_reporting_tool/DEreport_pe.Rmd ${REPORTS};
            fi &&
            
            PROJECT=\$(basename ${SHINYREPS_PROJECT})                            &&
            sed -i "2,2s/SHINYREPS_PROJECT/\${PROJECT}/" ${REPORTS}/DEreport_pe.Rmd &&
