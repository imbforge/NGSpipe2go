...
```{r process_RNA, echo=F, message=FALSE, error=TRUE, warning=TRUE}
...
```{r DNA_accessibility, echo=F, message=FALSE, error=TRUE, warning=TRUE}
...
patchwork::wrap_plots(WNNumap_CT, WNNumap, ncol = 2)
...
```{r cell_types_markerbased, echo=F, message=FALSE, error=TRUE, warning=TRUE, eval=TRUE}
...
sobj@meta.data$CTAnnotationSCType = ""
for(j in unique(sctype_scores$cluster)){
  cl_type = sctype_scores[sctype_scores$cluster==j,]; 
  sobj@meta.data$CTAnnotationSCType[sobj@meta.data$clusters_wnn == j] = as.character(cl_type$type[1])
}

WNNumap_celltypeannot <- DimPlot(sobj, reduction = "umap.wnn", label = TRUE, repel = TRUE, group.by = 'CTAnnotationSCType') + ggtitle("WNN clustering (celltype)")
ggsave(plot=WNNumap_celltypeannot, filename="report_files/umap_anno_WNNcluster_celltypeannotated.pdf")

patchwork::wrap_plots(WNNumap_celltypeannot, WNNumap, ncol = 2)
...
```{r atac_differentially_accessible_peaks}
...
```{r atac_differentially_accessible_peaks_by_celltype, echo=F, message=FALSE, error=F, warning=F, eval=F}

## differential accessibility between pairs of samples per calculated per cluster

ATAC_DA_MINCELLS <- 20        # minimum number of cells to include cluster in analysis

sobj$Group_Celltypes <- paste(sobj$group, sobj$predicted.id, sep = "_")
...
for (cl in sort(unique(sobj$predicted.id))) {
  for (sp in 1:ncol(sample_pairs)) {
    grp1 <- paste(sample_pairs[1, sp], cl, sep="_")
    grp2 <- paste(sample_pairs[2, sp], cl, sep="_")
    if (grp1 %in% names(grp_counts) && grp_counts[grp1] >= ATAC_DA_MINCELLS && 
...
cat(paste("\nLook for enriched motifs in differentially accessible peaks filtered for p_val_adj <", ATAC_DA_PLIM, "(motif enrichment is skipped if less than 3 peaks)\t"))
...
saveRDS(sobj, file = file.path("report_files", "R_sobj_merged.RDS"))
##sobj <- readRDS(file = file.path("report_files", "R_sobj_merged.RDS"))
