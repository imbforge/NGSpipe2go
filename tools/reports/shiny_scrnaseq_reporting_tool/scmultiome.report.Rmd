...
```{r cell_types_markerbased, fig.height=10, echo=F, message=FALSE, error=TRUE, warning=TRUE}
...
Identify differentially accessible peaks between all pairs of groups for each cluster (see full results in 'peaks_diffacces_clusters.xlsx').
...
Identify differentially accessible peaks between all pairs of groups for each celltype (see full results in 'peaks_diffacces_celltypes.xlsx').
...
List of motifs is obtained from Bioconductors `JASPAR2020` package. This code is implemented for human and mouse.

Identify enriched motifs in differentially accessible peaks per cluster (see full results in 'motif_enriched_clusters.xlsx').
...
for (i in names(da_enriched_motifs)) {
  cat(paste("\nComparison", i, "\n"))
  print(kbl(da_enriched_motifs[[i]][max(10,nrow(da_enriched_motifs[[i]])), ], caption=paste("Enriched motifs (top 10) for differentially accessible peaks between pairs of groups per cluster:", i), format="simple", align = "l", row.names=T) %>% kable_styling(full_width = F))
}

```

Identify enriched motifs in differentially accessible peaks per celltype (see full results in 'motif_enriched_celltypes.xlsx').
...
for (i in names(da_enriched_motifs_ct)) {
  cat(paste("\nComparison", i, "\n"))
  print(kbl(da_enriched_motifs_ct[[i]][max(10,nrow(da_enriched_motifs_ct[[i]])), ], caption=paste("Enriched motifs (top 10) for differentially accessible peaks between pairs of groups per celltype:", i), format="simple", align = "l", row.names=T) %>% kable_styling(full_width = F))
}
...
## Motif activity between clusters (NOT YET IMPLEMENTED)

### Per-cell motif activity (NOT YET IMPLEMENTED)
...
### Calculate differential motif activity between clusters (NOT YET IMPLEMENTED)
