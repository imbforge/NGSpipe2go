...
# If you want to exclude samples according to the given certain pattern (instead of including), define it in excludePattern. 
...
samplePattern=SHINYREPS_SAMPLEPATTERN1 
excludePattern=SHINYREPS_SAMPLEPATTERN2 
maxno=as.numeric(SHINYREPS_MAXNO)        # default NULL
##### 

cat(DEhelper.Fastqc(web=F, samplePattern=samplePattern, excludePattern=excludePattern, maxno=maxno), sep="\n")
...
# Applying FastQC again after adapter trimming
samplePattern=SHINYREPS_SAMPLEPATTERN2
maxno=as.numeric(SHINYREPS_MAXNO) # default NULL
##
cat("### FastQC after adapter trimming\n")
cat(DEhelper.Fastqc(web=F, subdir="trimmed", samplePattern=samplePattern, maxno=maxno), sep="\n") 
```
...
cat(DEhelper.geneBodyCov(web=F, samplePattern=SHINYREPS_SAMPLEPATTERN1), sep="\n")
```
...
cat(DEhelper.strandspecificity(samplePattern=SHINYREPS_SAMPLEPATTERN1), sep="\n")
```
...
```{r Qualimap}
DEhelper.Qualimap(samplePattern=SHINYREPS_SAMPLEPATTERN1)
```
...
DEhelper.RNAtypes(samplePattern=SHINYREPS_SAMPLEPATTERN1)
```
...
In the following plot only relevant contaminants are shown. A contaminant is considered relevant if it consumes `r SHINYREPS_FASTQSCREEN_PERC`% of reads or more in at least one sample. The "one genome" label refers to the amount of reads mapping to that genome only regardlessly if they are multi mapping or uniquely mapping reads. The "multiple genome" label refers to the amount of read mapping to this genome but also to other genomes in the screening regardless if they are multi mapping on the genome or uniquely mapping to that genome. 
...
cat(DEhelper.dupRadar(web=F, samplePattern=SHINYREPS_SAMPLEPATTERN1), sep="\n")
```
...
The plots below visualize the count data distribution (not log-transformed) via plate position for the categories library size, detected features and percentage of mitochondrial reads indicated by spot size. 0-cell and 10-cell controls are indicated in blue and orange, respectively. Symbol shape is defined by custom grouping. For each category the plates are plotted row-wise with plate number in increasing order.\n\n\n")
...
if(type_of_threshold=="absolute") { ### use absolute thresholds
  ...
  qc.drop$cells <- if("cells" %in% colnames(colData(sce))) {colData(sce)$cells != "1c"} else {F}
  ...
  if(!is.null(annoFactors)) { # prepare cell counts by factor if present
    data_by_factor <- cbind(qc.drop, tempfactor=factor(colData(sce)[,annoFactors])) %>%
      ...
      dplyr::summarize(counts_unfiltered = paste0(length(libsize), " (", sprintf("%1.1f%%", 100*length(libsize)/ncol(sce)), ")"),
                       ...
                       detected = paste0(sum(detected), " (", sprintf("%1.1f%%", 100*sum(detected)/length(detected)), ")"),
                       mito = paste0(sum(mito), " (", sprintf("%1.1f%%", 100*sum(mito)/length(mito)), ")"),
                       cells = paste0(sum(cells), " (", sprintf("%1.1f%%", 100*sum(cells)/length(cells)), ")"),
...
if(!"cells" %in% colnames(colData(sce))) { # exclude row with control wells if not applicable
  qcfailed <- qcfailed[!rownames(qcfailed) %in% c("cells"), ]
}
...
qc.drop.mod <- qc.drop[,!colnames(qc.drop) %in% c("pass", "cells")]
...
plotPCAfromQCmetrics(sce, metrics=qcmetrics, anno=annoFactors, qc.drop=qc.drop[,!colnames(qc.drop) %in% c("cells")])
...
if(!"cells" %in% colnames(colData(sce))) {x$cells <- NULL}
...
list_igraph_clusterassign <- lapply(sce_igraphclust, table)
igraph_clusterassign <- plyr::ldply(list_igraph_clusterassign, rbind) %>%
  #replace(is.na(.), 0) %>%
...
Heatmap(igraph_clusterassign, cluster_rows = F, cluster_columns = F, name="#cells",
        ...
        column_names_side="top", column_names_max_height = unit(7, "cm"), 
...
# cat("\nOverview cluster assignments:\n", fill=T) # outdated separate tables per clustering algorithm
# for (x in names(sce_igraphclust)) {
#   print(kbl(t(table(sce_igraphclust[[x]])), align = "l", format="markdown", caption=x) %>% kable_styling(full_width = F)) # format="simple"
# }
...
# prepare single table with cell numbers per cluster for each clustering setting to display as heatmap
list_hclust_clusterassign <- lapply(clust_sce, table)
hclust_clusterassign <- plyr::ldply(list_hclust_clusterassign, rbind) %>%
  #replace(is.na(.), 0) %>%
  column_to_rownames('.id') %>%
  t()  
...
```{r select_clustering, results='asis'}
  ...
  plot_selected_annofactor <- scater::plotReducedDim(sce, 
                      dimred = name_dimred,
                      by_exprs_values = exprs_values,
                      colour_by=colorByFactor2[1], point_size=plot_pointsize) +
                      #ggtitle(paste(gsub("_", " ", name_dimred), "colored by", colorByFactor2[1])) + 
                      theme(legend.position = "top", aspect.ratio = 1)
