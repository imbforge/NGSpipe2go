...
cat(sum(is.na(sce$phases)), "out of", length(sce$phases),
    "cells could not be assigned to a cell cycle phase and are removed from the dataset.", fill=TRUE)
...
```{r blocking_on_the_cell_cycle_phase, eval=any(!is.na(sce$phases))}
...
for(type in names(hvg.cor.list)){
    if(!is.null(hvg.cor.list[[type]]$cor.plot)) {
...
for(type in names(hvg.cor.list)){
  ...
  if(type=="all"){
    ...
    sce.tmp <- sce[, sce$type==type]
    do.call(multiplot, c(lapply(c(3, 5), function(p) {
        set.seed(100)
...
for(type in names(hvg.cor.list)){
  ...
  if(type=="all"){
    ...
    sce.tmp <- sce[, sce$type==type]
    title_suf <- paste("for group:", type)
  }
  ...
  umap1 <- plotUMAP(sce.tmp,  rerun = T, 
              ...
              ggtitle(paste("UMAP of all genes", title_suf))
  
  set.seed(100)
  umap2 <- plotUMAP(sce.tmp, rerun = T, 
                    colour_by=dotcol, shape_by=dotshape,
                    run_args=list(ntop=nrow(sce.tmp), feature_set=top.hvg)) + # feature_set overwrites ntop
              ggtitle(paste("UMAP of cor HVGs", title_suf))
