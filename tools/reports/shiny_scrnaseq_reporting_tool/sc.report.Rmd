...
plot(DEhelper.fastqscreen(samplePattern=SHINYREPS_SAMPLEPATTERN1))
...
if(length(hvg) == 0) {
  ...
  #                        colour_by=dotcol,
  #                        shape_by=dotshape) +
  #           geom_boxplot(color = "darkgrey", alpha = 0.2) +
...
write.table(data.frame(cell_ID=colnames(sce), colData(sce)[,c(group.vars, grep("sc3_", colnames(colData(sce)), value=T))]), 
            file =file.path(dir_sc3, paste0("SC3_", nr_clusters, "_cluster_assignments.txt")), sep="\t", quote = F, row.names = F)
...
if (!file.exists(dir_igraph)) {dir.create(dir_igraph, recursive=T) }
      write.table(data.frame(cell_ID=colnames(sce), colData(sce)[,c(group.vars, grep("igraph", names(colData(sce)), value=T))]), 
...
if (!file.exists(dir_hclust)) {dir.create(dir_hclust, recursive=T) }
      write.table(data.frame(cell_ID=colnames(sce), colData(sce)[,c(group.vars, grep("hclust", names(colData(sce)), value=T))]), 
