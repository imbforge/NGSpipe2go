...
not_yet_attached <- attach_packages(c("tidyverse", "AnnotationDbi", "batchelor", "Biobase", "BiocSingular", "bluster", "Cairo", "celldex", 
          "cluster", "corrplot", "dendextend", "DESeq2", "dynamicTreeCut", "edgeR", "GeneOverlap", "ggbeeswarm", "ggrepel", 
          "gplots", "grid", "gridExtra", "igraph", "kableExtra", "knitr", "limma", "M3Drop", "Matrix", "parallel", 
...
# load list of mitochondrial genes (if not given, we will use all genes annotated on mitochondrial chromosome)
...
switch(SHINYREPS_SEQTYPE,
    ...
    # Columns required: "sample", "barcode", "row", "col", "cells", "group", "pool"   
      ...
      colorByFactor2 <- "group" # default for downstream plots (up to 2 categories)
   }, 
    SmartSeq2={
    ...
    tenX={
      ...
      colorByFactor2 <- "sample" # default for downstream plots (up to 2 categories)
   },
...
samplePattern="cutadapt" # "_R1\\."
excludePattern=T  # default FALSE
maxno=30        # default NULL
##### 
...
# targets$sample must be a substring of file name). 
...
cat(DEhelper.Fastqc(web=F, samplePattern="cutadapt", exclude=excludePattern, maxno=maxno), sep="\n") 
```
...
```{r strand_specificity, echo=F, results='asis', error=F, warning=F, message=F}
...
```{r total_counts_per_plate_position, fig.width=8, eval= (SHINYREPS_SEQTYPE %in% c("MARSseq", "SmartSeq2"))}
##### parameters to set:
...
for (size in c("Lib.size", "detected", "subsets_Mito_percent")) {
  ...
  gridExtra::grid.arrange(grobs=plates, layout=matrix(c(1:ceiling(length(plates))), ncol=2, byrow=TRUE))
}
...
qcmetrics = c("sum", "detected", "percent.top_2", "subsets_Mito_percent")
...
if(SHINYREPS_TYPE_OF_THRESHOLD=="absolute") {
  ...
  kable(qcfailed, format="markdown", caption="QC filtering") %>% kable_styling()
    ...
    if(SHINYREPS_TYPE_OF_THRESHOLD=="relative") { # Consider experimental factors with batch=factor
     qc.drop$libsize <- isOutlier(sce$sum, nmads=NMADS, type="lower", log=TRUE) # type="both
     qc.drop$detected <- isOutlier(sce$detected, nmads=NMADS, type="lower", log=TRUE) 
     qc.drop$mito <- isOutlier(sce$subsets_Mito_percent, nmads=NMADS, type="higher")
     qc.drop$pass <- !apply(qc.drop, 1, any)
     qc.drop <- qc.drop[match(colnames(sce), rownames(qc.drop)), ]  # sort to match the 'sce' cell order
...
# to.plot <- data.frame(
#     DeconvFactor=calculateSumFactors(sce, clusters=qclust),
#     SpikeFactor=sizeFactors(sce),
#     colData(sce)[,annoFactors, drop=F]
# )
# ggplot(to.plot, aes(x=DeconvFactor, y=SpikeFactor, color=!!dotcol, shape=!!dotshape)) +
#     geom_point() + scale_x_log10() + scale_y_log10() + geom_abline(intercept=0, slope=1, color="red")
...
to.plot <- data.frame(DeconvFactor=sizeFactors(sce),
                 total_counts=sce$sum/1e6,
                 colData(sce)[,annoFactors, drop=F])
ggplot(to.plot, aes(x=DeconvFactor, 
                    y=total_counts,
...
```{r plot_violin_top50, fig.width=12, fig.height=10, eval=T}
##### parameters to set:
# define up to 2 categories to be indicated in the plot by color and shape.
annoFactors <- colorByFactor2
#####
...
if (org %in% c("human", "mouse")) {
  ...
  cat("\nCell cycle classification is available for human and mouse only.\n")
}
...
explanatoryVariables <- c(colorByFactor2, if("phases" %in% colnames(colData(sce))){"phases"} else {NULL})
#####
...
lapply(explanatoryVariables, function(x) {
...
## Adjust for confounder variables
...
# define up to 2 categories to be indicated in the plot by color and shape.
annoFactors <- colorByFactor2 
ncomponents=2  # number of principal components to obtain. Later clustering will be based on the these reduced dim slots.  
...
# print cluster assignment
  knitr::kable(t(table(colData(sce)[, paste0("sc3_", nr_clusters, "_clusters")])), align = "l", format="simple", caption="cells per cluster") %>% kable_styling(full_width = F) 
...
for (x in names(sce_igraphclust)) {
  print(kable(t(table(sce_igraphclust[[x]])), align = "l", format="simple", caption=x) %>% kable_styling(full_width = F))
  cat("\n")
}
...
for (x in names(clust_sce)) {
  print(kable(t(table(clust_sce[[x]])), align = "l", format="simple", caption=x) %>% kable_styling(full_width = F))
  cat("\n")
}
...
```{r select_clustering}
  ...
  cat("\nSelected clustering:", selected_clustering_method, "\nreducedDimName:", name_dimred, "\n\n")
...
for (i in levels(colData(sce)[,selected_clustering_method])) {
    ...
    print(kable(go.useful[1:10,], align = "l", format="simple") %>% kable_styling(full_width = F))
...
for (i in levels(colData(sce)[,selected_clustering_method])) {
    ...
    print(kable(as.data.frame(topTags(res)), align = "l", format="simple") %>% kable_styling(full_width = F)) 
...
```{r doublet_detection_by_simulation, results='asis'}
cat("# Doublet detection by simulation\n")

cat("\nThis doublet detection strategy involves in silico simulation of doublets from the single-cell expression profiles (Dahlin et al. 2018). This is performed using the computeDoubletDensity() function from the scDblFinder package.\n")
...
TSNE_DoubletScore <- plotReducedDim(sce, dimred=name_dimred, colour_by="DoubletScore")
TSNE_DoubletScore

plotCluster_DoubletScore <- plotColData(sce, x=selected_clustering_method, y="DoubletScore", colour_by = selected_clustering_method)
