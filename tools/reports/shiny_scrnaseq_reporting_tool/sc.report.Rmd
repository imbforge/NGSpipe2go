...
plot_pointsize <- 0.6 # point size for plotReducedDim plots
plot_pointshape <- 16 # point shape for plotReducedDim plots (if not occupied by parameter)
plot_pointalpha <- 0.4 # point transparency for plotReducedDim plots
...
switch(SHINYREPS_SEQTYPE,
    ...
    tenX={
      ...
      if(!is.na(SHINYREPS_RUN_DEMUX)) { # sample multiplexing applied?
        ...
        if(SHINYREPS_RUN_DEMUX=="demux_GT") { # add information if sample multiplexing by genetic variance is applied
          ...
          # but note that in case the assignment of subsamples to the sample ids in targets_pools is arbitrary 
   ...
   ParseBio= { # Demux not necessary because this is already implemented in the assay.
      # define matrix directory. DGE_filtered subdirectory contains only the cells that passed the pipeline’s minimum transcript count threshold.
      matrix_dir = file.path(SHINYREPS_SPLITPIPECOMB_OUT, "all-sample/DGE_filtered/")
      # load targets.txt (data per fastq file). Columns required: "sample", "file", "group", "replicate"
      targets_pools <- read.delim(SHINYREPS_TARGET, sep="\t", comment.char = "#")
      targets_pools_sumBySample <- targets_pools %>% # summarize multiple rows per sample induced by distribution of samples across fastq files
        group_by(sample) %>%
        summarise(across(everything(), ~ paste(unique(.), collapse = ", "))) %>%
        as.data.frame()
      # create targets on cell level 
      targets = read.delim(file.path(matrix_dir, "cell_metadata.csv"), header = T, stringsAsFactors = FALSE, sep=",") 
      names(targets)[1] = "cell_id" 
      # merge targets with targets_pools_sumBySample and sort 
      targets <- plyr::join(targets, targets_pools_sumBySample, by="sample", type="left") 
      rownames(targets) <- targets$cell_id
      StartingColumnNames <- c("cell_id", "sample", "file", "group", "replicate")
      targets <- targets[order(targets$sample, targets$cell_id), c(StartingColumnNames, colnames(targets)[!colnames(targets) %in% StartingColumnNames])]
      ParseBioColumns <- c("wells", "species", "gene_count", "tscp_count", "mread_count", colnames(targets)[grepl("(wind)|(well)$", colnames(targets))])
      group.vars <- colnames(targets)[!colnames(targets) %in% c("cell_id", "sample", "file", ParseBioColumns)] 
      targets[,group.vars] <- lapply(targets[,group.vars], factor)
      group.vars <- group.vars[sapply(group.vars, function(x) {length(unique(targets[,x]))>1})] # remove group vars with single value
...
# Otherwise pruned file names will be used by default.
#####
if (SHINYREPS_SEQTYPE %in% c("tenX", "ParseBio")) {
  cat("\nMapping Statistics can be found in the respective web summary.\n")
} else {
...
switch(SHINYREPS_SEQTYPE,
    ...
    ParseBio={
      # matrix_dir already defined above 
      barcode.path <- file.path(matrix_dir, "cell_metadata.csv")
      barcode.names <- read.delim(barcode.path, header = T, sep=",", stringsAsFactors = FALSE)
      features.path <- file.path(matrix_dir, "all_genes.csv")
      feature.names = read.delim(features.path, header = T, sep=",", stringsAsFactors = FALSE)
      matrix.path <- file.path(matrix_dir, "count_matrix.mtx")
      counts <- Matrix::readMM(file = matrix.path) # cells in rows, genes in columns!
      counts <- t(counts)
      colnames(counts) = barcode.names$bc_wells
      rownames(counts) = feature.names$gene_id
      counts <- counts[,targets$cell_id] # subset cell barcodes by targets (e.g. for successful cell hashing)
    },
    stop(c("Don't find seqtype:", SHINYREPS_SEQTYPE))   
)
...
if (!is.na(mito.genes)) {
 ...
 is.mito <- gsub(".*_", "", rowData(sce)$CHR) %in% c("chrM", "chrMT", "M", "MT") # some assays have prefixes like 'mm39_' in chr names    
...
if(any(!is.na(sce$phases))) {
  ...
  # print cell cycle table
   ccptable <- data.frame(tempfactor=factor(colData(sce)[, c(annoFactors[1])]), phases=sce$phases) %>%
    ...
    points(assignments$scores$G1, assignments$scores$G2M, col=scales::alpha(pal[factor(colData(sce)[,annoFactors[1]])], .5), 
           pch=plot_pointshape, cex=plot_pointsize )   
...
cat("\nPCA plots of top 500 genes colored by potential confounder variables\n\n")  
  ...
  explVarScatter <- lapply(explanatoryVariables, function(x) {
                    scater::plotPCASCE(sce, ncomponents = 4, point_size=plot_pointsize, point_alpha=plot_pointalpha, colour_by=x) + 
...
# plot PCA
  gridExtra::grid.arrange(grobs=lapply(grep("PCA", reducedDimNames(sce), value=T), function(d) {
      ...
      scater::plotReducedDim(sce, dimred = d, 
        ...
        colour_by=dotcol, shape_by = dotshape, point_size=plot_pointsize, point_alpha=plot_pointalpha) +  
...
```{r sc3, results="asis", eval=F}
## sc3 not used by default anymore

library(SC3)
library(pkgmaker)

cat("## Defining clusters with SC3
...
set.seed(100)
sce <- sc3_prepare(sce, gene_filter=F, n_cores=8, rand_seed = 100) # 
set.seed(100)
      ...
      # dim reduction plots
        plot_cluster <- mclapply(plot_Items, function(i) {
                  scater::plotReducedDim(sceSC3,  # use sceSC3 instead of sce
                    ...
                    guides(color = guide_legend(ncol=2, override.aes = list(size=1, alpha=1)), 
                           shape = guide_legend(override.aes = list(size=1, alpha=1)))
        ...
        for (cl in names(sce_igraphclust)) { # cluster results to color
          ...
          # dim reduction plots
            plot_cluster[[paste(cl, p, sep="_")]] <- invisible(scater::plotReducedDim(sce, 
                        ...
                        shape_by = dotshape, point_size=plot_pointsize, point_alpha=plot_pointalpha) +
              scale_color_discrete(name="cluster") +
              guides(color=guide_legend(ncol=2, override.aes = list(size=1, alpha=1)),
                     shape = guide_legend(override.aes = list(size=1, alpha=1))) + 
              ggtitle(paste("Graph-based clustering on", gsub("_", " ", cl), "\nillustrated by", gsub("_", " ", p))) +
              theme(plot.title = element_text(size = 9), aspect.ratio = 1))
...
######################
  ...
  plot_selected <- scater::plotReducedDim(sce, 
                      dimred = name_dimred,
                      by_exprs_values = exprs_values,
                      colour_by=selected_clustering_method, point_size=plot_pointsize, point_alpha=plot_pointalpha) +  
                      scale_color_discrete(name="cluster") +
                      #guides(color=guide_legend(ncol=2)) +
  ...
  plot_selected_annofactor <- scater::plotReducedDim(sce, 
                      dimred = name_dimred,
                      by_exprs_values = exprs_values,
                      colour_by=annoFactors[1], point_size=plot_pointsize, point_alpha=plot_pointalpha) +
                      #guides(color=guide_legend(ncol=2)) +
...
```{r doublet_detection_with_clusters, results='asis', eval=SHINYREPS_SEQTYPE %in% c("tenX", "ParseBio")}
library(scDblFinder)
...
```{r doublet_detection_by_simulation, results='asis', eval=SHINYREPS_SEQTYPE %in% c("tenX", "ParseBio")}
annoFactors <- colorByFactor2
######################
...
pairwise.de <- apply(conts,1,function(cont) {
  ...
  summedCont <- summed[, summed$diffgroups %in% factors] # select only samples included in the respective contrast
  summedCont$diffgroups <- factor(summedCont$diffgroups)
  summedCont$diffgroups <- relevel(summedCont$diffgroups, factors[2]) # relevel to make sure the comparison is done in the right direction (B vs A and not A vs B)
    ...
    for (i in levels(colData(summedCont)[,selected_clustering_method])) {
        ...
        # The factors[2] group is selected as the reference level since factors[2] is the first level in diffgroups
        design <- try(
