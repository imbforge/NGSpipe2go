...
# load list of mitochondrial genes (not needed, instead we use all genes starting with "MT-")
...
switch(SHINYREPS_SEQTYPE,
    ...
    # Columns required: "sample", "barcode", "row", "col", "cells", "group", "pool"   
      ...
      # create targets_pools from targets
...
if(SHINYREPS_TYPE_OF_THRESHOLD=="absolute") {
  ...
  kable(qcfailed, format="markdown", caption="QC filtering") %>% kable_styling()
    ...
    if(SHINYREPS_TYPE_OF_THRESHOLD=="relative") {
     qc.drop$libsize <- isOutlier(sce$sum, nmads=NMADS, type="lower", log=TRUE)
...
explanatoryVariables <- c(colorByFactor2, "phases")
var2color <- c(colorByFactor2, "phases")
#####
...
# eval= (!is.null(var2adjust) | adjust_ccp)
...
```{r cell_type_annotation, results='asis', message=F,  eval=(org %in% c("human", "mouse"))}
library(SingleR)
...
if(org=="human") {
  ...
  pred <- SingleR(test=countdata, ref=ref, labels=ref$label.main) # alternative: ref$label.fine
  #table(pred$labels)
...
DE_var2adjust <- "final_var2adjust" # "pool" # NULL
...
# Creating pseudo-bulk samples from clusters uses (unnormalised "counts" matrix)
  ...
  summed <- aggregateAcrossCells(sce, ids=colData(sce)[,c(selected_clustering_method, varDiffGroups)], #  
                use.assay.type = "counts")
...
#multiplot(plotlist=list(TSNE_DoubletScore, plotCluster_DoubletScore), cols=1)
