...
attach_packages(c("rmarkdown", "knitr", "Cairo", "gplots", "ggplot2", "plotly", "RColorBrewer", "ggrepel", "parallel",
                  "grid", "gridExtra", "rtracklayer", "scater", "scran", "scde", "ggbeeswarm", "dplyr", "tidyr", "forcats",
...
} else { # end load counts for MARSseq design
     
  if(seqtype == "SmartSeq2") {
...
## load counts
  ...
  colnames(counts) <- gsub(lcSuffix(samples),"", basename(f))
  # colnames(counts) <- gsub(SHINYREPS_PREFIX,"", colnames(counts))

# get indices of loaded (pruned) filenames looked up in target$filename 
  index.list <- sapply(colnames(counts), grep, target$filename) 
# check if all loaded (pruned) filenames are found exactly once in targets$filenames
  if(any(sapply(index.list, length) != 1)) {stop(paste("count file names not found in targets$filenames:", names(which(sapply(index.list, length) != 1))))}
  if(any(duplicated(index.list))) {stop(paste("count file names not mapped uniquely to targets$filnames:", names(index.list[which(duplicated(index.list))])))}

# filer targets for count file names loaded
  targets <- targets[unlist(index.list),]

  # end load counts for SMARTseq design
    } else { 
...
if(type_of_threshold=="absolute") {
  libsize.drop <- (sce$total_counts < threshold_total_counts_min) | (sce$total_counts > min(threshold_total_counts_max, max(sce$total_counts)))
  ...
  if(!is.null(threshold_total_counts_max)) {threshold_total_counts_max <- paste("or >", threshold_total_counts_max)} # prepare string for output
...
cat("\ncell cycle phases")
table(sce$phases)
cat(sum(is.na(sce$phases)), "cells could not be assigned to a cell cycle phase and are removed from the dataset.")
...
assignments$score <- assignments$score[!is.na(sce$phases), ] # NA entries are not used
design <- model.matrix(~ G1 + G2M, assignments$score)

if(!exists("sce_incl_NA_phase")) {sce_incl_NA_phase <- sce} 
 sce <- sce[,!is.na(sce$phases)] # remove cells which could not be classified
...
rm(sce.no_block) # not needed any more
  ...
  do.call(multiplot, c(mclapply(c(5, 10), function(p) {
    ...
    plotTSNE(sce.tmp,  rerun = T, 
             ...
             run_args=list(perplexity=p,
                           ntop=nrow(sce.tmp), # feature_set overwrites ntop
                           feature_set=top.hvg)) +
...
for(type in names(hvg.cor.list)){
  ...
  umap1 <- plotUMAP(sce.tmp,  rerun = T, 
             colour_by=dotcol, shape_by=dotshape,
             run_args=list(ntop=nrow(sce.tmp) # ntop default is 500
                           )) +  
  ...
  umap2 <- plotUMAP(sce.tmp, rerun = T, 
             colour_by=dotcol, shape_by=dotshape,
             run_args=list(ntop=nrow(sce.tmp), 
