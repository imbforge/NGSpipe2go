...
mito.genes <- tryCatch(read.delim(SHINYREPS_MTGENES)[, 1], error=function(e) NA)
...
switch(SHINYREPS_SEQTYPE,
    ...
    SmartSeq2={
        ...
        colnames(counts) <- gsub(lcSuffix(basename(f)),"", basename(f))

        targets <- targets[match(colnames(counts), targets$sample, nomatch=0), ]  # subset targets to match only samples with counts files
    },
    stop(c("Don't find seqtype:", SHINYREPS_SEQTYPE))   # Otherwise
)
...
if(SHINYREPS_TYPE_OF_THRESHOLD=="absolute") {
  libsize.drop <- (sce$total_counts < threshold_total_counts_min) | (sce$total_counts > min(threshold_total_counts_max, max(sce$total_counts), na.rm=T))
  ...
  if(is.na(threshold_total_counts_max)) {threshold_total_counts_max <- paste("or >", threshold_total_counts_max)} # prepare string for output
  ...
  qcfailed <- data.frame(criterion=c(paste("total counts <", threshold_total_counts_min, threshold_total_counts_max),
                                     ...
                                     paste("remaining")), 
                         cell_count=c(sum(libsize.drop), sum(feature.drop), sum(mito.drop), sum(keep)))
...
pca <- cbind(pca, as.data.frame(colData(sce)[match(rownames(pca), colnames(sce)), unique(c(annoFactors, "cells")), drop=F]))

## extract all dropped samples
#df <- subset(pca, libsize | feature | mito | libsize.high)
df <- subset(pca, libsize | feature | mito)
df$libsize.word      <- ifelse(df$libsize==TRUE,"lib size","")
...
if(!is.null(annoFactors)) {
  if(length(annoFactors) !=1) {annoFactors <- annoFactors[1]} # just 1 annoFactor can be illustrated
  if(annoFactors %in% c("group", "group_name")) {
      annoFactors.name <- paste0(annoFactors, ".1")} else { # the terms "group" and "group_name" are occupied by runPCA
      annoFactors.name <- annoFactors}  
  annoFactors.name <- rlang::ensym(annoFactors.name)
  } else {
  annoFactors.name <- NULL}
  
p <- plotReducedDim(pca.sce, use_dimred="PCA_coldata", by_exprs_values = "counts", shape_by=annoFactors) +
...
if(nrow(df.kept) > 0)
    p <- p + geom_point(data=df.kept, aes(x=PC1, y=PC2, color="kept", shape=!!annoFactors.name), size=3)
...
```{r limma_blocking_confounder, eval=any(!is.na(sce$phases))}
...
```{r explanatory_variables_2, echo=F, error=F, warning=F, message=F, fig.width=8, fig.height=6, results='asis', eval=T}
...
write.table(colData(sce)[,c(group.vars2, grep("sc3_", colnames(colData(sce)), value=T))], file =file.path(dir_sc3, paste0("SC3_", nr_clusters, "_cluster.txt")), sep="\t", quote = F, row.names = F)
...
if (!file.exists(dir_igraph)) {dir.create(dir_igraph, recursive=T) }
      write.table(colData(sce)[,c(group.vars2, grep("igraph", names(colData(sce)), value=T))], 
...
if (!file.exists(dir_hclust)) {dir.create(dir_hclust, recursive=T) }
      write.table(colData(sce)[,c(group.vars2, grep("hclust", names(colData(sce)), value=T))], 
...
```{r findmarkers, fig.width=11, fig.height=20, results='asis'}
...
dir_marker <- file.path("report_files", "marker_per_cluster", selected_clustering_method)
...
markers <- findMarkers(sce, groups=colData(sce)[,selected_clustering_method], 
                       block= if (is.null(DE_var2adjust)) {NULL} else {colData(sce)[,DE_var2adjust]},  
                       assay.type = "logcounts_not_adjusted",
                       row.data=rowData(sce)[,"SYMBOL", drop=F], direction="up", lfc=1)
markers_spec <- findMarkers(sce, groups=colData(sce)[,selected_clustering_method], 
                       block= if (is.null(DE_var2adjust)) {NULL} else {colData(sce)[,DE_var2adjust]},  
                       assay.type = "logcounts_not_adjusted",
...
```{r cell_type_annotation, results='asis', eval=T}
library(SingleR)
# eval= org %in% c("human", "mouse")
...
# Extract symbols for each GO term; done once.
    tab <- select(org.Mm.eg.db, keytype="ENSEMBL", keys=rownames(sce), columns="GOALL")
