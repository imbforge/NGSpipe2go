    ...
    tenX={
      ...
      if(!is.na(SHINYREPS_RUN_DEMUX)) { # sample multiplexing applied?
        ...
        if(SHINYREPS_RUN_DEMUX=="demux_GT") { # add information if sample multiplexing by genetic variance is applied
          ...
          names(overviewStatus) <- aggrcsvPrunedSampleId
          overviewStatus <- as.data.frame(dplyr::bind_rows(overviewStatus, .id = "file"))
      ...
      targets_pools$file <- gsub("(_S.)*(_L...)*(_R.)*(_001)*", "", targets_pools$file) # in case file name was pruned in targets_pools
