...
switch(SHINYREPS_SEQTYPE,
      ...
      if(!"sample" %in% colnames(targets4plots)) {# "sample" column is needed. Use pruned "file" column if ambiguous.
        targets4plots$sample <- gsub("(_S\\d{1,3}$)|(_S\\d{1,3}_L\\d{3}_R\\d_\\d{3}$)", "", targets4plots$file)
        } 
       },
      ScaleBio={      
        targets4plots <- targets_pools |> # summarize multiple rows per file for fastq-centered QC plots
        dplyr::group_by(file) |>
        dplyr::summarise(dplyr::across(dplyr::everything(), ~ paste(unique(.), collapse = ", "))) |>
        as.data.frame()
        # include fastqc plots provided by manufacturer
        SHINYREPS_FASTQC_OUT <- file.path(SHINYREPS_EXTERNAL_RESULTDIR, "fastq/fastqc")
},
      ParseBio={
        targets4plots <- targets_pools |> # summarize multiple rows per file for fastq-centered QC plots
        dplyr::group_by(file) |>
...
if(file.exists(SHINYREPS_FASTQC_OUT)) {
  ...
  numberFastqcPlots <- fastqc_summary_plot$no.of.samples

} else {
  cat("\nNo fastqc files found.\n")
  numberFastqcPlots <- nrow(targets4plots)

}
```

```{r FastQC_qual, echo=F,results='asis',error=F,warning=F,message=F,fig.align='center',fig.height=min(170, round((3*numberFastqcPlots)/4)+1), eval=file.exists(SHINYREPS_FASTQC_OUT)}
...
```{r FastQC_nuc_content_summarized, echo=F,results='asis',error=F,warning=F,message=F,fig.align='center', fig.height=min(170, ceiling(0.12*numberFastqcPlots)+1), eval=file.exists(SHINYREPS_FASTQC_OUT)}
if (SHINYREPS_FASTQC_SUMMARIZED) {
...
```{r FastQC_nuc_content, echo=F,results='asis',error=F,warning=F,message=F,fig.align='center',fig.height=min(170, round((3*numberFastqcPlots)/4)+1), eval=file.exists(SHINYREPS_FASTQC_OUT)}
...
```{r FastQC_GC, echo=F,results='asis',error=F,warning=F,message=F,fig.align='center', eval=file.exists(SHINYREPS_FASTQC_OUT)}
...
cat("\nThe following plot shows the amount of reads trimmed for the selected adapter sequences including polyA/polyT sequences if specified. The column 'tooshort' gives the percentage of reads removed due to a length of less than the specified minimum after trimming. Additionally to the column 'trimmed', which is for all adapters combined, there are columns for every single adapter giving the percentage of reads trimmed for this adapter.\n\n")

# define categories of targets file to be used for dot color in the plot (one plot per element of colorByFactor will be created). 
DEhelper.cutadapt(colorByFactor=c("group"), targetsdf=targets4plots)

```

```{r FastQC_qual_summarized_trimmed, echo=F,results='asis',error=F,warning=F,message=F,fig.align='center', eval=runCutadapt & file.exists(file.path(SHINYREPS_FASTQC_OUT, "trimmed"))}
...
```{r FastQC_qual_trimmed, echo=F,results='asis',error=F,warning=F,message=F,fig.align='center',fig.height=min(170, round((3*numberFastqcPlots)/4)+1), eval=runCutadapt & file.exists(file.path(SHINYREPS_FASTQC_OUT, "trimmed"))}
...
```{r FastQC_nuc_content_summarized_trimmed, echo=F,results='asis',error=F,warning=F,message=F,fig.align='center', fig.height=min(170, ceiling(0.12*numberFastqcPlots)+1), eval=runCutadapt & file.exists(file.path(SHINYREPS_FASTQC_OUT, "trimmed"))}
...
```{r FastQC_nuc_content_trimmed, echo=F,results='asis',error=F,warning=F,message=F,fig.align='center',fig.height=min(170, round((3*numberFastqcPlots)/4)+1), eval=runCutadapt & file.exists(file.path(SHINYREPS_FASTQC_OUT, "trimmed"))}
...
```{r FastQC_GC_trimmed, echo=F,results='asis',error=F,warning=F,message=F,fig.align='center', eval=runCutadapt & file.exists(file.path(SHINYREPS_FASTQC_OUT, "trimmed"))}
...
```{r GeneBodyCoverage_paragraph, echo=F, error=F, warning=F, message=F}
...
```{r GeneBodyCoverage_paragraph_plotly, echo=F, error=F, warning=F, message=F, results='asis', fig.align="center", eval="plotly" %in% names(plot_list)}

cat("#### Gene body coverage per file: plotly")
cat("\n\n", fill=T)
plot_list$plotly |>
...
if("p_per_replicate_splitByGroup" %in% names(plot_list)){
  cat("#### Gene body coverage split by group, colored by replicate: \n", fill=T)
...
if("p_per_group_splitByReplicate" %in% names(plot_list)){
  cat("#### Gene body coverage split by replicate, colored by group: \n", fill=T)
...
```{r Qualimap, fig.height=min(170, ceiling(0.12*nrow(targets4plots))+1)}
...
```{r RNAtypes_paragraph, echo=F, results='asis', error=F, warning=F, message=F, fig.height=min(170, ceiling(0.12*nrow(targets4plots))+1)}
...
switch(SHINYREPS_SEQTYPE,
       tenX={
         ...
         # collect all summary tables per file
         pathMappingFiles <- list.files(file.path(SHINYREPS_CELLRANGERCOUNT_OUT), pattern = "metrics_summary.csv$", full.names = TRUE, recursive = TRUE)
         
         if(length(pathMappingFiles)>0) {
           names(pathMappingFiles) <- gsub(paste(SHINYREPS_CELLRANGERCOUNT_OUT, "/outs/metrics_summary.csv", "/" , sep = "|"), "", pathMappingFiles)
           
           mappingStats <- readr::read_csv(pathMappingFiles)  
           rownames(mappingStats) <- names(pathMappingFiles)
           mappingStats <- t(mappingStats)
           print(kableExtra::kbl(mappingStats, caption="Per sample mapping Statistics after processing with proprietary software from manufacturer", row.names=T, format="simple", align = "l") |> kableExtra::kable_styling())
           
           # check if stats from cellranger_aggr are available as well
           pathMappingAggr <- file.path(SHINYREPS_CELLRANGERAGGR_OUT, SHINYREPS_CELLRANGERAGGR_ID, "outs", "count", "summary.json")
           if(file.exists(pathMappingAggr)) {
             
             mappingStatsAggr <- jsonlite::fromJSON(pathMappingAggr, flatten=TRUE)
             mappingStatsAggr[grepl("chemistry", names(mappingStatsAggr))] <- NULL
             mappingStatsAggr[sapply(mappingStatsAggr, length)!=1] <- NULL
             mappingStatsAggr <- tibble::enframe(unlist(mappingStatsAggr, use.names = TRUE), name = "category", value = "value") |>
               dplyr::mutate(dplyr::across(where(is.numeric), ~ formatC(.x, format = "f", digits = 2)))
             print(kableExtra::kbl(mappingStatsAggr, caption="Aggregated mapping Statistics after processing with proprietary software from manufacturer", row.names=F, format="simple", align = "l") |> kableExtra::kable_styling())
           }
         } else {
       ...
       ParseBio={
         pathMappingStats <- file.path(SHINYREPS_SPLITPIPECOMB_OUT, "agg_samp_ana_summary.csv")
       ...
       ScaleBio={
             pathMappingStats <- file.path(SHINYREPS_EXTERNAL_RESULTDIR, "/reports/allSamples.reportStatistics.csv")

             if(file.exists(pathMappingStats)) {
               mappingStats <- readr::read_csv(pathMappingStats) |>
                 dplyr::filter(Category != "Extrapolated Complexity") |>
                 dplyr::rename_with(~stringr::str_remove(.x, "\\.ScaleRNA"))
               print(kableExtra::kbl(mappingStats, caption="Mapping Statistics after processing with proprietary software from manufacturer", row.names=F, format="simple", align = "l") |> kableExtra::kable_styling())
                 } else {
                   print("Mapping Statistics can be found in the respective web summary provided by manufacturer.")
                 }
       ...
       SmartSeq={
           ...
           star_stats_violin <- DEhelper.STAR.violin(colorByFactor=c("group"), targetsdf=targets4plots)
           cat("\n#### Summary\n", fill=T)
             for(i in 1:length(star_stats_violin$p_mapped)){
               gridExtra::grid.arrange(grobs=star_stats_violin$p_mapped[[i]], layout_matrix= matrix(c(1,2), nrow=1)) 
             }
           cat("\n", fill = T)
           cat("\n#### Mapping statistics\n", fill=T)
...
cat("\nQC parameter is indicated by symbol size and sample group by shape. '0C' controls are highligthed in blue, '10c' controls in red.")
...
Uniform manifold approximation and projection (UMAP) is a nonlinear dimensionality reduction technique to identify sub-populations in expression data. Here, UMAP dimensions are calculated with", paste(SHINYREPS_N_NEIGHBORS, collapse=", "), "nearest neighbors.\n")
...
# Remark: the explanatory_variables_density_plot.png is not shown due to redundancy
...
if(length(cl_avail)>0) {
  ...
  cat("\nWe perform a gene enrichment analysis (over-representation) on the marker genes defining each cluster. This identifies the processes that are (relatively) active in each cluster based on up-regulation of the associated genes compared to other clusters. We use gene sets defined by the Gene Ontology (GO) project, which describe a comprehensive range of biological processes and functions. We use the top", SHINYREPS_TOPMARKER4GO, "marker genes and apply the goana function from the limma package. This performs a hypergeometric test to identify GO terms that are overrepresented in our marker subset. We keep only biological process terms that are not overly general (<=500 genes) and which are significantly enriched", paste0("(p<", SHINYREPS_MARKERGO_PTHRESHOLD,")."), "\n\n")
...
The DE analysis will be performed using quasi-likelihood (QL) methods from the edgeR package (Robinson, McCarthy, and Smyth 2010; Chen, Lun, and Smyth 2016). This uses a negative binomial generalized linear model (NB GLM) to handle overdispersed count data in experiments with limited replication. We test for differences in expression using the glmQLFTest function. 
...
if(length(anno_avail)>0) {
  ...
  tab <- purrr::pmap(params, function(cn, de) {
    
    overviewFileName <- list.files(file.path(SHINYREPS_DE_OUT, de), pattern = paste0("^de_.*", cn, "_overview_sign_genes\\.txt$"), full.names = TRUE)
    
    if(length(overviewFileName)==1 && file.exists(overviewFileName)) {
    
      cat(paste("\n\n###", de, "for contrast:", cn, "\n")) 
  
      overviewFile <- overviewFileName |>
        readr::read_tsv() |>
        dplyr::arrange(dplyr::pick(1)) |>
...
if(length(de_avail)>0) {
  ...
  cat("\nWe perform a gene set enrichment analysis on the DE genes defining each cluster. This identifies the pathways and processes that are (relatively) active in each cluster based on upregulation of the associated genes compared to other clusters. We will use gene sets defined by the Gene Ontology (GO) project, which describe a comprehensive range of biological processes and functions. We use the top", SHINYREPS_TOPDE4GO, "DE genes and apply the goana function from the limma package using", gsub("DE", "DE ", SHINYREPS_DEGO_UNIVERSE), "genes as background. This performs a hypergeometric test to identify GO terms that are overrepresented in our gene subset of significantly differentially expressed genes. We keep only biological process terms that are not overly general (<=500 genes) and which are significantly enriched", paste0("(p<", SHINYREPS_DEGO_PTHRESHOLD,")."), "\n\n")
