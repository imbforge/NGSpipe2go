...
loadGlobalVars()
load(paste0(SHINYREPS_DE_DESEQ_DIR,"/",SHINYREPS_SMALLRNATYPE,"/",SHINYREPS_DE_DESEQ_FILE), envir=.GlobalEnv)  # the outcome from the DE analysis
``` 
...
cat(paste0("\n\n Only ",SHINYREPS_SMALLRNATYPE, " genes are used for the following analysis.\n"),fill=T)
...
Here, only `r SHINYREPS_SMALLRNATYPE` genes were selected and a summary of what percentage of the overall assigned reads is assigned to them is given below.

```{r subread_paragraph, echo=F, results='asis', error=F, warning=F, message=F}
subread <- smallRNAhelper.subread.type(subdir=SHINYREPS_SMALLRNATYPE,maindir="all")
