...
plot_list <- DEhelper.geneBodyCov2(web=F, samplePattern=SHINYREPS_SAMPLEPATTERN1, excludePattern=SHINYREPS_SAMPLEPATTERN2, maxno=SHINYREPS_MAXNO)
...
DEhelper.strandspecificity(samplePattern=SHINYREPS_SAMPLEPATTERN1, excludePattern=SHINYREPS_SAMPLEPATTERN2, maxno=SHINYREPS_MAXNO)
```
...
DEhelper.Qualimap(samplePattern=SHINYREPS_SAMPLEPATTERN1, excludePattern=SHINYREPS_SAMPLEPATTERN2, maxno=SHINYREPS_MAXNO)
```
...
DEhelper.RNAtypes(samplePattern=SHINYREPS_SAMPLEPATTERN1, excludePattern=SHINYREPS_SAMPLEPATTERN2, maxno=SHINYREPS_MAXNO)
```
...
# Otherwise pruned file names will be used by default.
#####
cat("\n10X mapping statistics can be found in the cellranger web summary.\n")

cat("\nMapping of the STARR mRNAs to the STARR library genome & transcriptome is performed with STAR (https://github.com/alexdobin/STAR). The program version, genome assembly and software parameters are described in the table at the end of the report.
...
cat("Mapping of the STARR mRNAs to the STARR library genome & transcriptome is performed with STAR (https://github.com/alexdobin/STAR). The program version, genome assembly and software parameters are described in the table at the end of the report.
...
```{r umicount, echo=F, results='asis', error=F, warning=F, message=F, eval=T}
##### parameters to set:
...
cat(DEhelper.dupRadar(web=F, samplePattern=SHINYREPS_SAMPLEPATTERN1, excludePattern=SHINYREPS_SAMPLEPATTERN2, maxno=SHINYREPS_MAXNO), sep="\n")
```
...
#### load SingleCellExperiment object created by pipeline
sce <- readRDS(SHINYREPS_SCE)
...
```{r doublet_detection_with_clusters, results='asis', eval=T}
library(scDblFinder)
...
```{r doublet_detection_by_simulation, results='asis', eval=T}
annoFactors <- colorByFactor2
######################
