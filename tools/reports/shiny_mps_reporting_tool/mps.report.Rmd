...
run_cutadapt <- dir.exists(SHINYREPS_CUTADAPT_LOGS)
...
#
samplePattern=SHINYREPS_PATTERNFASTQC2
maxno=SHINYREPS_MAXNO        
##### 

cat(DEhelper.Fastqc(web=F, samplePattern=samplePattern, exclude=T, maxno=maxno), sep="\n")
```
...
MPShelper.cutadapt(targetsdf=NULL, colorByFactor=NULL)
```



```{r FastQC_paragraph2, echo=F, results='asis', error=F, warning=F, message=F, eval=run_cutadapt}
##### parameters to set:
# Select samples for which you would like to include fastqc results in the report. For huge experiments, 
# you may want to restrict the total number of plots. If you provide a regular expression in 'samplePattern' only those 
# filenames will be included which match this expression, e.g. setting samplePattern="R1" yields only those fastq 
...
MPShelper.umiextract(samplePattern=samplePattern, exclude=T, colorByFactor=colorByFactor, targetsdf=targets)
```
