---
title: "um_ryu_2017_01_castillo_RNA_seq"
output:
  html_document:
    toc: true
    toc_float: true
...
For many genes in the human, mouse & other genomes, RNA-seq with short reads cannot measure expression accurately because recently duplicated gene families, as well as some gene-pseudogene pairs, are very similar on transcript level and few reads map uniquely to only one family member. There are several approaches how to deal with multimapped (MM), here such reads are randomly assigned to one of the several genes they may come from and then the DESeq2 statistical analysis is repeated. While this alternative analysis may reveal additional interesting hits, which would otherwise be lost, it may also produce false-positive findings due to potential "cross-talk" between highly similar genes, i.e. if a gene X changes expression, its pseudogene Y or closely-related paralog Z may be non-specifically affected as well. Therefore, the standard analysis using only uniquely mapped reads is usually more reliable (specific) for the majority of genes and should have priority.
