...
for(i in 1:nrow(conts)) {
    ...
    bar <- paste0( SHINYREPS_GO_ENRICHMENT, "/", gsub("=.*", "", conts[i,1]),"_GO_Barplot.png")
    if(file.exists(bar)) {
    cat("Gene Ontology enrichment barplot of the differentially expressed genes", fill=T)
    cat("![GO_Analysis img](", bar, ")",fill=T)
    cat("\n", fill=T)
    
    # GO enrichment network 
    network <- paste0(SHINYREPS_GO_ENRICHMENT, "/", gsub("=.*", "", conts[i,1]), "_GO_network.png")
    cat("Gene Ontology enrichment map of the differentially expressed genes",fill=T)
    cat("![GO_Analysis img](", network,")",fill=T)
    cat("\n", fill=T)
   
    # and GO enrichment table
     GOtable <- paste0(SHINYREPS_GO_ENRICHMENT, "/", gsub("=.*", "",conts[i,1]),"_GO_Enrichment.csv")
     cat("Gene Ontology enrichment table of the differentially expressed gene. The table shows the enriched biological proccesses \n",fill=T)
     enrich <- read.csv(file=GOtable)
     cols <- c("ID","Description","GeneRatio","BgRatio","pvalue","p.adjust","qvalue","geneID")
     r <- min(10,nrow(enrich))
     enrich <- as.data.frame(enrich)
     if(nchar(enrich$geneID)>10){
     enrich$geneID <- paste0(substr(enrich$geneID, 0, 35),"...")
     } 
     cat(kable(enrich[1:r,cols], row.names=T, align="r"), sep="\n") # show only the top min or top 10 biological processes
      cat("\n", fill=T)
     } else {
      cat("no GO enrichment result available",fill=T)
       cat("\n", fill=T)
    }
