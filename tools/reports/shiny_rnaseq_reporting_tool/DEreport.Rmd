...
## Mapping to rRNAs and other potential contaminants:
...
## Distribution of reads on exons, introns and intergenic regions ##
...
Note that by default a standard DESeq2 analysis uses only a statistical (FDR) filter and is thus testing for evidence that the expression changes between two sample groups is different from zero. Performing post-hoc filtering on fold changes is not recommended by the software developers and should be avoided. Such a "Volcano plot" filtering approach may result in inflated false discovery rates as reported in https://doi.org/10.1093/bib/bbab053. If desirable, the differential expression analysis can incorporate an additional fold change (FC) threshold directly in the DESeq2 statistical testing model, which results in more conservative but accurate DEG estimates compared to post-hoc filtering.

For the analyses below, the FDR filter was set to `r SHINYREPS_DE_DESEQ_FDR` and the FC filter was set to `r SHINYREPS_DE_DESEQ_FC` (note: FC=1 means no FC filter was used).
...
Differential Splicing analysis is performed with the rMATS software suite, which uses the RNA-Seq data to automatically detect and analyze all major types of alternative splicing patterns. The statistical model of rMATS calculates the false discovery rate that the difference in the isoform ratio of a gene between two conditions exceeds an user-defined threshold (Default: 0.01%).

The maser (Mapping Alternative Splicing Events to pRoteins) package is used to enable the functional characterization of splicing in transcriptomic contexts. In a nutshell, maser allows a detailed analysis of splicing events identified by rMATS by filtering the rMATS splicing events based on RNA-seq coverage, p-values and differential percent spliced-in (PSI), analysis of global splicing effects using principal component analysis and volcano plots, mapping of splicing events to Ensembl transcripts and visualization of transcripts affected by splicing using custom Gviz plots.

The following abbreviations describe the different types of alternative splicing events:
...
Low coverage splicing junctions are commonly found in RNA-seq data and lead to low confidence PSI levels. We remove low coverage events with coverage < `r SHINYREPS_MASER_MINCOV` reads, which may significantly reduced the number of splicing events. Statistically significant events are selected using a FDR cutoff of `r SHINYREPS_MASER_FDR` and minimum PSI change of `r as.numeric(SHINYREPS_MASER_DPSI)*100`% (deltaPSI = `r SHINYREPS_MASER_DPSI`).
