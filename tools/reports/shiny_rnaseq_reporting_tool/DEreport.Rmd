...
for(i in 1:nrow(conts)) {
	...
	cat("MA plot showing expression fold change vs. mean expression level of all annotated genes. There are", sum(genes$padj < fdr, na.rm=T), 
	...
	cat("Volcano plot of all DESeq2-tested genes depicting their expression fold change vs. statistical significance. The FDR < 0.01 hits are highlighted.", "\n", "\n")
	...
	cat("##### Top hits", fill=T)
        cat("", fill=T)
	cat("The table below shows the top 25 DEGs with their mean normalised counts (baseMean), expression change (log2FoldChange) and statistical significance (padj = FDR-adjusted p-value). The complete DESeq2 results table including RPKM-normalised expression data for all genes in each sample is available in CSV and Excel formats.", fill=T)
	cat("", fill=T)
