...
```{r STAR_paragraph, echo=F, results='asis', error=F, warning=F, message=F, fig.align="center", out.width="100%", fig.height=(0.2*fastqc_summary_plot$no.of.samples)+4}
...
A competitive mapping to the reference genome and known rRNAs (H.sapiens, M.musculus, C.elegans, D.melanogaster, D.rerio, X.tropicalis) as well as frequently occuring Mycoplasma species (M. arginini, M. fermentans, M. hominis, M. hyorhinis,M. orale, and Acholeplasma laidlawii), PhiX and ERCC spike-in sequences, E.coli and B.taurus was performed with FastQScreen (http://www.bioinformatics.babraham.ac.uk/projects/fastq_screen/).

In the following plot the "one_genome" label refers to the amount of reads mapping to that genome only reagardlessly if they are multi mapping or uniquely mapping reads.
...
Since the reads mapping to mulitple genomes are refered to for each individual genome the total pecentages of reads mapping can be larger than a 100%. 
...
cat("\n\n")
#then we plot per sample
cat("### Gene body coverage per sample: ")
cat("\n\n", fill=T)
plot_list$per_sample
cat("\n\n", fill=T)
if("per_group" %in% names(plot_list)){
...
In this  anlysis the FC filter was `r SHINYREPS_DE_DESEQ_FC` and the FDR filter was `r SHINYREPS_DE_DESEQ_FDR`.  
...
for(i in 1:nrow(conts)) {
    ...
    #since only genes with a mininaml foldchange of FC are
...
for(i in 1:nrow(conts)) {
    ...
    if(file.exists(GO_bar_up)) {
        ...
        cat("Gene Ontology Biological Processes (all) of the up-regulated genes with FDR < 0.01, grouped by similarity. Color and area represent the cluster and the significance (-log10 adjusted p-value) of the enrihcment, respectively.", fill=T)
    ...
    if(file.exists(GO_bar_down)) {
        ...
        # GO enriched terms in a treemap
        cat("Gene Ontology Biological Processes (all) of the down-regulated genes with FDR < 0.01, grouped by similarity. Color and area represent the cluster and the significance (-log10 adjusted p-value) of the enrihcment, respectively.", fill=T)
