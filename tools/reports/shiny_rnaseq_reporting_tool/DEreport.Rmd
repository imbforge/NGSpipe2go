...
for(i in 1:nrow(conts)) {
    ...
    # GO Enrichment Plot
    cat("#### GO Enrichment plot", fill=T)

    GO_bar_up     <- paste0(SHINYREPS_GO_ENRICHMENT, "/", gsub("=.*", "", conts[i,1]),"_GO_Barplot_upregulated_genes.png")
    GO_network_up <- paste0(SHINYREPS_GO_ENRICHMENT, "/", gsub("=.*", "", conts[i, 1]), "_GO_network_upregulated_genes.png")
    GO_table_up   <- paste0(SHINYREPS_GO_ENRICHMENT, "/", gsub("=.*", "", conts[i, 1]), "_GO_Enrichment_upregulated_genes.csv")
    ...
    if(file.exists(GO_bar_up)) {
        ...
        # network of GO enriched terms
        cat("Gene Ontology enrichment map of the up-regulated genes with FDR < 0.01.", fill=T)
        cat("![GO_Analysis img](", GO_network_up, ")\n", fill=T)
    ...
    GO_bar_down     <- paste0(SHINYREPS_GO_ENRICHMENT, "/", gsub("=.*", "", conts[i,1]),"_GO_Barplot_downregulated_genes.png")
    GO_network_down <- paste0(SHINYREPS_GO_ENRICHMENT, "/", gsub("=.*", "", conts[i, 1]), "_GO_network_downregulated_genes.png")
    GO_table_down   <- paste0(SHINYREPS_GO_ENRICHMENT, "/", gsub("=.*", "", conts[i, 1]), "_GO_Enrichment_downregulated_genes.csv")
    ...
    if(file.exists(GO_bar_down)) {
        ...
        # network of GO enriched terms
        cat("Gene Ontology enrichment map of the down-regulated genes with FDR < 0.01.", fill=T)
        cat("![GO_Analysis img](", GO_network_down, ")\n", fill=T)
    ...
    cat("#### Pathway Enrichment plot", fill=T)

    Pathway_bar_up     <- paste0(SHINYREPS_GO_ENRICHMENT, "/", gsub("=.*", "", conts[i,1]),"_Reactome_Barplot_upregulated_genes.png")
    Pathway_network_up <- paste0(SHINYREPS_GO_ENRICHMENT, "/", gsub("=.*", "", conts[i, 1]), "_Reactome_network_upregulated_genes.png")
    Pathway_table_up   <- paste0(SHINYREPS_GO_ENRICHMENT, "/", gsub("=.*", "", conts[i, 1]), "_Reactome_Pathway_Enrichment_upregulated_genes.csv")
    ...
    if(file.exists(Pathway_bar_up)) {
        ...
        # network of Pathways enriched
        cat("Pathway enrichment map of the up-regulated genes with FDR < 0.01.", fill=T)
        cat("![GO_Analysis img](", Pathway_network_up, ")\n", fill=T)
    ...
    Pathway_bar_down     <- paste0(SHINYREPS_GO_ENRICHMENT, "/", gsub("=.*", "", conts[i,1]),"_Reactome_Barplot_downregulated_genes.png")
    Pathway_network_down <- paste0(SHINYREPS_GO_ENRICHMENT, "/", gsub("=.*", "", conts[i, 1]), "_Reactome_network_downregulated_genes.png")
    Pathway_table_down   <- paste0(SHINYREPS_GO_ENRICHMENT, "/", gsub("=.*", "", conts[i, 1]), "_Reactome_Pathway_Enrichment_downregulated_genes.csv")
    ...
    if(file.exists(Pathway_bar_down)) {
        ...
        # network of Pathways enriched
        cat("Pathway enrichment map of the down-regulated genes with FDR < 0.01.", fill=T)
        cat("![GO_Analysis img](", Pathway_network_down, ")\n", fill=T)
