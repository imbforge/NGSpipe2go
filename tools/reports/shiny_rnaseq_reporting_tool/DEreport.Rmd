...
For the analyses below, the FC filter was set to `r SHINYREPS_DE_DESEQ_FC` (note: FC=1 means no FC filter) and the FDR filter was set to `r SHINYREPS_DE_DESEQ_FDR`.
...
The R/Bioconductor package clusterProfiler (https://bioconductor.org/packages/release/bioc/html/clusterProfiler.html) was used to find enriched GO terms and pathways in the lists of significantly up- and down-regulated genes from the pairwise differential expression analyses. The resulting enrichment tables and plots can be found in folder "GO_Analysis". Shown below are several complementary visualisations aimed to help understanding the underlying biological changes and the mutual relationships between the enriched categories.
