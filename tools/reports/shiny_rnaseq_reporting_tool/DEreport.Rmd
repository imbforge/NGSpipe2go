...
for(i in 1:nrow(conts)) {
    ...
    cat("Gene Ontology enrichment barplot of the differentially expressed genes ", fill=T)
    paste0("![GO_Analysis img](", SHINYREPS_GO_ENRICHMENT, "/", conts[i,1], "_GO_Barplot.png", ")")
    cat("\n",fill=T)

    # GO enrichment network 
    cat("Gene Ontology enrichment map of the differentially expressed genes", fill=T)
    paste0("![GO_Analysis img](", SHINYREPS_GO_ENRICHMENT, "/", conts[i,1], "_GO_network.png",")")
    cat("\n",fill=T)

    # and GO enrichment table
    cat("Gene Ontology enrichment table of the differentially expressed gene. The table shows the top 10 enriched biological processes", fill=T)
    cat("",fill=T)
    cat("####" conts[i,1])
    paste("![GO_Analysis csv](", SHINYREPS_GO_ENRICHMENT, "/", conts[i,1], "GO_Enrichment.csv",")")
    cols <- c("ID","Description","GeneRatio","BgRatio","pvalue","p.adjust","qvalue","geneID")
    cat(kable(enrich[1:10,cols], row.names=T, align="r"), sep="\n") # show only the top 10 biological processes
    cat("",fill=T)
...
for(i in 1:nrow(conts)) {
    ...
    # and DE table
    cat("The table contains contains a summary of the likelihood that there are more DE genes in a particular chromosome than expected by chance. The p-value is calculated with Fisher's exact text. This test also gives an odds ratio which represents the strength of association. If an odds ratio is equal to or less than 1, there is no association between the two lists. If the odds ratio is much larger than 1, then the association is strong. The class also calculates the Jaccard index which measures the similarity between two lists. The Jaccard index varies between 0 and 1, with 0 meaning there is no similarity between the two and 1 meaning the two are identical.", fill=T)
    cat("", fill=T)
...
The Venn diagrams below compare the number of differentially expressed genes without and with inclusion of the ambiguous multimapped (MM) redas.
