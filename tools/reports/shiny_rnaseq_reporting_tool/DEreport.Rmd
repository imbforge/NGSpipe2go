...
The "GC content" plot shows the GC% distribution of all reads and the ideal normal distribution based on the data (black line). Typically, the lines should tightly overlap and look essentially the same in all samples. An unusually shaped distribution could indicate a contaminated library.
...
for(i in 1:nrow(conts)) {
	...
	# if there are GO enrichment results (the barplot was created)
    	if(file.exists(GO_bar_up)) {
        	...
        	cat(kable(enrich[1:r, cols], row.names=T, align="r", format="markdown"), sep="\n") # show only the top min or top 10 biological processes
        	cat("\n", fill=T)
