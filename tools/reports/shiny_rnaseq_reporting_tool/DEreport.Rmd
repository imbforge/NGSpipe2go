...
```{r GeneBodyCoverage_paragraph, echo=F, error=F, warning=F, message=F, results='asis'}
plot_list <- DEhelper.geneBodyCov2(web=F)
cat("### Genebodycoverage per sample: plotly")
cat("\n\n", fill=T)
plot_list$plotly %>%
    highlight(dynamic=T, selectize=T, persistent=T) 
cat("\n\n")
#then we plot per sample
cat("### Genebodycoverage per sample: ")
cat("\n\n", fill=T)
plot_list$per_sample
cat("\n\n", fill=T)
if("per_group" %in% names(plot_list)){
  cat("### Genebodycoverage per sample group : ")
  plot_list$per_group
}
```
