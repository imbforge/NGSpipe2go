...
for(i in 1:nrow(conts)) {
    ...
    for(s in sections) {
      ...
      GO_bar     <- paste0(SHINYREPS_GO_ENRICHMENT, "/", conts$contrast.name[i], paste0("_GO_barplot", s$suffix, ".png"))
      GO_network <- paste0(SHINYREPS_GO_ENRICHMENT, "/", conts$contrast.name[i], paste0("_GO_network", s$suffix, ".png"))
      GO_treemap <- paste0(SHINYREPS_GO_ENRICHMENT, "/", conts$contrast.name[i], paste0("_GO_treemap", s$suffix, ".png"))
      GO_scatter <- paste0(SHINYREPS_GO_ENRICHMENT, "/", conts$contrast.name[i], paste0("_GO_scatterplot"     , s$suffix, ".png"))
      GO_dot     <- paste0(SHINYREPS_GO_ENRICHMENT, "/", conts$contrast.name[i], paste0("_reduced_GO_dotplot" , s$suffix, ".png"))
      GO_cnet    <- paste0(SHINYREPS_GO_ENRICHMENT, "/", conts$contrast.name[i], paste0("_reduced_GO_cnetplot", s$suffix, ".png"))

      # if there are GO enrichment results (the barplot was created)
      if(file.exists(GO_bar)) {
          cat("###### Barplot (all terms)\n", fill=T)
          cat("Top enriched Gene Ontology BP terms for the ", s$text, " genes:\n", fill=T)
          cat("![](", GO_bar, ")\n", fill=T)

          cat("###### Network (all terms)\n", fill=T)
          cat("Network plot of the top enriched Gene Ontology BP terms for the ", s$text, " genes (connecting lines represent shared genes):\n", fill=T)
          cat("![](", GO_network, ")\n", fill=T)

          cat("###### Treemap clusters\n", fill=T)
          cat("Clustering and reduction of the enriched Gene Ontology BP terms for the ", s$text, " genes (area size correlates with significance):\n", fill=T)
          cat("![](", GO_treemap, ")\n", fill=T)

          cat("###### Top terms clusters (reduced terms)\n", fill=T)
          cat("Top enriched Gene Ontology BP term clusters (reduced GO terms) for the ", s$text, " genes:\n", fill=T)
          cat("![](", GO_dot, ")\n", fill=T)

          cat("###### Term-gene network (reduced terms)\n", fill=T)
          cat("Top enriched Gene Ontology BP term clusters (reduced GO terms) with associated genes for the ", s$text, " genes:\n", fill=T)
          cat("![](", GO_cnet, ")\n", fill=T)
      } else {
          cat("no GO enrichment results for the ", s$text, " genes available.\n", fill=T)
      }
      ...
      Reactome_Pathway_bar <- paste0(SHINYREPS_GO_ENRICHMENT, "/", conts$contrast.name[i], paste0("_Reactome_barplot" , s$suffix, ".png"))
      Reactome_network     <- paste0(SHINYREPS_GO_ENRICHMENT, "/", conts$contrast.name[i], paste0("_Reactome_network" , s$suffix, ".png"))
      Reactome_cnet        <- paste0(SHINYREPS_GO_ENRICHMENT, "/", conts$contrast.name[i], paste0("_Reactome_cnetplot", s$suffix, ".png"))

      # if there are Reactome Pathway enrichment results (the barplot was created)
      if(file.exists(Reactome_Pathway_bar)) {
          cat("###### Barplot\n", fill=T)
          cat("Top enriched Reactome pathways for the ", s$text, " genes:\n", fill=T)
          cat("![](", Reactome_Pathway_bar, ")\n", fill=T)

          cat("###### Network\n", fill=T)
          cat("Network plot of the top enriched Reactome pathways for the ", s$text, " genes (connecting lines represent shared genes):\n", fill=T)
          cat("![](", Reactome_network, ")\n", fill=T)

          cat("###### Term-gene network\n", fill=T)
          cat("Top enriched Reactome pathways with associated genes for the ", s$text, " genes:\n", fill=T)
          cat("![](", Reactome_cnet, ")\n", fill=T)
      } else {
          cat("no Reactome Pathway enrichment results for the ", s$text, " genes available\n", fill=T)
      }

      cat("##### KEGG pathways {.tabset .tabset-pills}\n", fill=T)
      cat("KEGG pathways enriched in the list of ", s$text, " genes.\n", fill=T)
      KEGG_Pathway_bar <- paste0(SHINYREPS_GO_ENRICHMENT, "/", conts$contrast.name[i], paste0("_KEGG_barplot" , s$suffix, ".png"))
      KEGG_network     <- paste0(SHINYREPS_GO_ENRICHMENT, "/", conts$contrast.name[i], paste0("_KEGG_network" , s$suffix, ".png"))
      KEGG_cnet        <- paste0(SHINYREPS_GO_ENRICHMENT, "/", conts$contrast.name[i], paste0("_KEGG_cnetplot", s$suffix, ".png"))

      # if there are KEGG Pathway enrichment results (the barplot was created)
      if(file.exists(KEGG_Pathway_bar)) {
          cat("###### Barplot\n", fill=T)
          cat("Top enriched KEGG pathways for the ", s$text, " genes:\n", fill=T)
          cat("![](", KEGG_Pathway_bar, ")\n", fill=T)

          cat("###### Network\n", fill=T)
          cat("Network plot of the top enriched KEGG pathways for the ", s$text, " genes (connecting lines represent shared genes):\n", fill=T)
          cat("![](", KEGG_network, ")\n", fill=T)

          cat("###### Term-gene network\n", fill=T)
          cat("Top enriched KEGG pathways with associated genes for the ", s$text, " genes:\n", fill=T)
          cat("![](", KEGG_cnet, ")\n", fill=T)
      } else {
          cat("no KEGG Pathway enrichment results for the ", s$text, " genes available.\n", fill=T)
      }
