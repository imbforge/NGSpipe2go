...
cat(DEhelper.Fastqc(web=F, samplePattern="R1"), sep="\n")
...
The following plot shows the amount of reads trimmed for the selected adapter sequences as well as polyA/polyT sequences.  as well as the amount filtered due to a length of less than 30bp after trimming.

```{r cutadapt, echo=F, results='asis', error=F, warning=F, message=F}
DEhelper.cutadapt(samplePattern=NULL, colorByFactor=NULL)
```
...
If plot.groupwise is set TRUE, the mapping characteristics are plotted seperatedly for the grouping variabtles given in targets.txt.
...
cat(DEhelper.STAR(plot.groupwise=F), sep="\n")
#group.vars, targets und targets.prefix wird gebraucht in groupwise plotting --> ändern wie in cutadapt! 
...
sce.noQCmetrics <- sce # store unmodified sce
...
## Count distrbution per plate position
...
annoFactors <- rlang::ensym(annoFactors)
...
```{r pca1, fig.height=11, fig.width=15}
##### parameters to set:
...
annoFactors.name <- rlang::ensym(annoFactors)
...
knitr::kable(table(colData(sce)[, annoFactors]), caption="Amount of cells remaining after QC filtering") %>% kable_styling()
...
knitr::kable(table(colData(sce)[, annoFactors]), caption="Amount of cells remaining after filtering for control wells (0c/10c)") %>% kable_styling() 
...
sce.tmp <- sce
sce.tmp <- calculateQCMetrics(sce.tmp, exprs_values = "counts", feature_controls=list(Mt=row.names(sce.tmp) %in% mito.genes))

plotExplanatoryVariables(sce.tmp, variables=explanatoryVariables, exprs_values = "logcounts") + fontsize
   ...
   tmp.sce <- sce[,sce$type==type] # if desired to subset this plot for subgroups of cells
    ...
    plot(
      plotExpression(tmp.sce, 
                     gtf$gene_name[match(rownames(hvg)[1:10], gtf$gene_id)],
                     colour_by=dotcol,
                     shape_by=dotshape) + 
