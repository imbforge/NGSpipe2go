...
insert <- lapply(branches[1], function(dir) {
  ...
  if (!file.exists(file.path(SHINYREPS_INSERTSIZE, dir))) {
  	...
  	sample_length <- length(list.files(file.path(SHINYREPS_INSERTSIZE, dir), pattern = "insertsizemetrics.tsv$"))
...
The overlap of peaks is shown in distinct mode, i.e. overlaps are counted if they occur in the respective set but not in any other set. E.g. a combination 1 1 0 means a set of elements is both in set A and B, while not in C. Under this mode, the combination sets are mutually exclusive.

```{r peak_upset_paragraph, echo=F,results='asis',error=F,warning=F,message=F}
upset <- lapply(branches, function(peaksdir) {
  if(length(branches)>1) {cat(paste("### Peaks from", if(peaksdir==""){"default"} else {peaksdir}, "pipeline branch\n\n"))}
  ChIPhelper.UpSetPlot(peaksdir, Mode="distinct", targetsdf=NULL)
})
```



## Correlation of peaks per group ##

The overlap of peaks is shown in intersect mode and highlighted for better comparability of replicates within each group. Overlaps are counted if they occur in the respective set irrespective if they occur in any other set. E.g. a combination 1 1 0 means a set of elements is in set A and B, and they can also in C or not in C. Under this mode, the combination sets can overlap.

```{r peak_upset_paragraph_by_groups, echo=F,results='asis',error=F,warning=F,message=F}
upset <- lapply(branches, function(peaksdir) {
  if(length(branches)>1) {cat(paste("### Peaks from", if(peaksdir==""){"default"} else {peaksdir}, "pipeline branch\n\n"))}
  ChIPhelper.UpSetPlot(peaksdir, Mode="intersect", targetsdf=targets, addBarAnnotation=F)
})
```



```{r peak_venn_paragraph_combined, echo=F, results='asis', error=F, warning=F, message=F, fig.height= (nrow(targets)*2+5)/2.54}
