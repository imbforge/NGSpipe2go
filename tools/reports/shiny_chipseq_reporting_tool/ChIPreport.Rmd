...
branches <- c(unique="", dupmarked="unfilt") # value is sub-directory, name is file tag in case of subsetting  
...
nbranches <- names(branches)[1]
insert <- lapply(nbranches, function(sampleSubset) {
  if(length(nbranches)>1) {cat(paste("\n\n### Insert size metrics for bam files containing tag:", sampleSubset, "\n\n"))}

  if (!file.exists(SHINYREPS_INSERTSIZE)) {
  	cat("Insert size data not available.\n")
  	} else {
  	  
  	cat(ChIPhelper.insertsize(samplePattern=sampleSubset), sep="\n")
  	
  	cat("\nThe insert size distribution is given as histogram:\n\n")  
  	 
  	p <- ChIPhelper.insertsize.plot(samplePattern=sampleSubset)
  	sample_length <- length(selectSampleSubset(list.files(SHINYREPS_INSERTSIZE, pattern = "insertsizemetrics.tsv$"),
  	                        samplePattern=sampleSubset))
...
ipstrength <- lapply(branches, function(peaksdir) {
  if(length(branches)>1) {cat(paste("### Fingerprint plots from", if(peaksdir==""){"default result"} else {peaksdir}, "directory\n\n"))}
  cat(ChIPhelper.IPstrength(web=F, subdir=peaksdir),sep="\n")
})
```
...
blacklistfilt <- lapply(branches, function(peaksdir) {
  if(length(branches)>1) {cat(paste("\n\nBlacklist filter applied for peaks from", if(peaksdir==""){"default result"} else {peaksdir}, "directory:\n\n"))}
...
peaksAllBranches <- lapply(branches, function(peaksdir) {
  ...
  if(length(branches)>1) {cat(paste("### Peaks from", if(peaksdir==""){"default result"} else {peaksdir}, "directory\n\n"))}
...
venn <- lapply(branches, function(peaksdir) {
  if(length(branches)>1) {cat(paste("### Peaks from", if(peaksdir==""){"default result"} else {peaksdir}, "directory\n\n"))}
  ChIPhelper.VennDiagram(peaksdir)
})
```


```{r peak_venn_paragraph_combined, echo=F, results='asis', error=F, warning=F, message=F}
if(length(branches)>1) {
cat("\n\n### Overlay of peaks from different pipeline branches\n")  
...
venncombined <- lapply(names(peakscovcombReorder), function(x) {
    ...
    makeVennDiagram(peaks.ov,
                    ...
                    fill=brewer.pal(length(peakscovcombReorder[[x]]), "Accent")[1:length(peakscovcombReorder[[x]])]
                    )
    cat("\n", fill=T)
  }
 )
...
featureDist <- lapply(branches, function(peaksdir) {
  ...
  if(length(branches)>1) {cat(paste("\n\n#### Feature distribution from", if(peaksdir==""){"default result"} else {peaksdir}, "directory\n"))}
...
TFbindingDist <- lapply(branches, function(peaksdir) {
    ...
    if(length(branches)>1) {cat(paste("\n\n#### TF-binding loci distribution from", if(peaksdir==""){"default result"} else {peaksdir}, "directory\n"))}
...
peakscov <- lapply(branches, function(peaksdir) {
    if(length(branches)>1) {cat(paste("\n\n#### ChIP peaks coverage from", if(peaksdir==""){"default result"} else {peaksdir}, "directory\n"))}
    cat(ChIPhelper.peakAnnotationCoverage(web=F, subdir=peaksdir),sep="\n")
})
```


```{r peak_annotation_coverage_combined, echo=F, results='asis', error=F, warning=F, message=F}
if(length(branches)>1) {
cat("\n\n#### Overlay of peaks from different pipeline branches\n")  
...
plotcombined <- lapply(names(peakscovcombReorder), function(x) print(ChIPseeker::covplot(peakscovcombReorder[[x]], weightCol = "X.log10.pvalue.", 
...
UpSetplots <- lapply(branches, function(peaksdir) {
    if(length(branches)>1) {cat(paste("\n\n#### ChIP peaks from", if(peaksdir==""){"default result"} else {peaksdir}, "directory\n"))}
...
resultsenrich <- lapply(branches, function(peaksdir) {
    if(length(branches)>1) {cat(paste("\n\n### Enrichment results from", if(peaksdir==""){"default result"} else {peaksdir}, "directory\n"))}
...
diffplots <- lapply(branches, function(peaksdir) {
    if(length(branches)>1) {cat(paste("\n\n### Diffbind results from", if(peaksdir==""){"default result"} else {peaksdir}, "directory\n"))}
    invisible(ChIPhelper.diffbind(subdir=peaksdir))
})
```
