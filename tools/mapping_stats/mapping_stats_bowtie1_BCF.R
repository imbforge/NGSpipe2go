...
bams <- list.files(path=data.folder,pattern='*.bam$',full.names=TRUE)
...
stats$Sample <- gsub(".bam|.cutadapt|.highQ|.deduped|.trimmed", "", stats$Sample)
...
ggsave(paste(output.folder,"/totalReads.pdf",sep=""), plotter, height=7,width=10)
ggsave(paste(output.folder,"/totalReads.png",sep=""), plotter, height=7,width=10)
