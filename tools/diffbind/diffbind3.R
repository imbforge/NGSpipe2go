...
peaks <- sapply(targets$Peaks, function(x) {
                  tryCatch({
...
if(!all(peaks > 0)) {
  warning("Sample(s) ", paste(targets$SampleID[!(peaks > 0)], collapse=", "),
          " excluded from Diffbind because didn't have any peaks called")
  targets <- targets[peaks > 0, ] 
}

# and contrasts with "orphan" targets
valid_conts <- sapply(strsplit(conts$contrast, "\\W"), function(factors) {
  factors <- gsub("(^\\s+|\\s+$)", "", factors)  # remove leading spaces
  factors <- factors[factors != ""]
  all(factors %in% targets$Condition)
})

if(!all(valid_conts)) {
  warning("Contrast(s) ", paste(conts$contrast.name[!valid_conts], collapse=", "),
          " excluded from Diffbind because some factors didn't have a valid peakset")
  conts <- conts[valid_conts, ]
}

##
## helper functions
##
build_dba_object <- function(sampleSheet, config) {

  # create dba object with results from the peakcaller
  db <- dba(sampleSheet=sampleSheet, config=config)
  ...
  db <- tryCatch({
          dba.blacklist(db, blacklist=db$config$doBlacklist, greylist=db$config$doGreylist)  # crashes if greylist cannot be build
        }, error=function(e) {
          warning("Building of greylist failed, analysis is performed without greylist.")
          db <- dba.blacklist(db, blacklist=db$config$doBlacklist, greylist=FALSE) # apply blacklist only (doesn't crash if no blacklist found)
  ...
  db <- dba.normalize(db, method =db$config$AnalysisMethod,
                      normalize  =NORMALIZATION,
                      library    =LIBRARYSIZE, 
                      offsets    =FALSE,
                      bSubControl=SUBSTRACTCONTROL_FINAL,
                      background =FALSE)

  # return dba object
  list(db=db, db2=db2, infodb=infodb,
       parms=list(blacklist=blacklist_generated, greylist=greylist_generated, bSubControl=SUBSTRACTCONTROL_FINAL))
}

##
## QC part:
## It's done using all samples together
##

# Construct DBA object
db_all <- build_dba_object(sampleSheet=targets,
                           config     =data.frame(AnalysisMethod=ANALYSISMETHOD,
                                                  th            =FDR_TRESHOLD,
...
try({
  ggplot(plotdata, aes(x=group, y=width, fill=group)) + geom_boxplot(show.legend = FALSE) + 
...
# Note: need to add the contrasts and run the analysis
png(paste0(OUT, "/venn_plot_all_contrasts.png"), width = 150, height = 150, units = "mm", res=300)
try({
  # parse formula in cont and add contrast to the db object
  for (i in 1:nrow(conts)) {
    cont.form <- conts$contrast[i]
    mmatrix   <- conts$mmatrix[i]
  ...
  db <- dba.analyze(db)
  dba.plotVenn(db, main="Binding Site Overlaps Per Contrast", contrast=1:nrow(dba.show(db, bContrast=T)))
})
dev.off()


##
## DB analysis part (contrasts):
## It's done isolating the targets in the contrast from the rest
##
result <- list()
...
for (i in 1:nrow(conts)) {
  ...
  cat("Processing contrast", cont.name, fill=TRUE)

  # Construct DBA object
  x <- build_dba_object(sampleSheet=targets[targets$Condition %in% factors, ],
                        config     =data.frame(AnalysisMethod=ANALYSISMETHOD,
                                               th            =FDR_TRESHOLD,
                                               fragmentSize  =FRAGSIZE,
                                               doBlacklist   =BLACKLIST,
  ...
  # The major change in DiffBind version 3.0 is in how the data are modeled. In 
  # previous versions, a separate model was derived for each contrast, 
  # including data only for those samples present in the contrast. Model design 
  # options were implicit and limited to either a single factor, or 
  # a subset of two-factor "blocked" designs. Starting in version 3.0, the 
  # default mode is to include all the data in a single model, allowing 
  ...
  try(dba.plotMA(db, fold=FOLD))
  dev.off()

  png(paste0(OUT, "/", cont.name, "_boxplot_diff_sites.png"), width = 150, height = 150, units = "mm", res=300)
  try(dba.plotBox(db))   # try, in case there are no significant peaks
  dev.off()

  png(paste0(OUT, "/", cont.name, "_volcano_plot.png"), width = 150, height = 150, units = "mm", res=300)
  rep <- try(dba.report(db, th=1)) # scale dot size by Conc (max dot size set to 4)
  try(dba.plotVolcano(db, fold=FOLD, dotSize=4*rep$Conc/max(rep$Conc)))
  dev.off()

  png(paste0(OUT, "/", cont.name, "_correlation_heatmap.png"), width = 150, height = 150, units = "mm", res=300)
  try(dba.plotHeatmap(db))   # try, in case there are no significant peaks
  dev.off()

  png(paste0(OUT, "/", cont.name, "_pca_plot.png"), width = 150, height = 150, units = "mm", res=300)
  try(dba.plotPCA(db, label=DBA_ID))   # try, in case there are no significant peaks
  dev.off()
  
  # plot consensus peaks
  # 1st group name between diffbind versions
  vennmask <- dba.mask(db, DBA_CONDITION, factors)
  if(sum(vennmask) <= 4) {   # if less than 2 replicates per group (or 4 replicates in total)
    ...
    dev.off()
     ...
     vennmask2 <- dba.mask(db2, DBA_CONDITION, factors) # db2 see above
     png(paste0(OUT, "/", cont.name, "_venn_plot.png"), width = 150, height = 150, units = "mm", res=300)
     try({
       dba.plotVenn(db2, main="Binding Site Overlaps Per Group",
                    mask=db2$masks$Consensus & names(db2$masks$Consensus) %in% factors)
     })
     dev.off()
  }

  result[[i]] <- tryCatch({
     dba.report(db, bCalled=T, bUsePval=F, th=1, fold=0) # th=db$config$th, fold=FOLD # filtering is done later
...
if(NORMALIZATION == "default") {
  explNormalization <- paste0("refers to method '", infoNorm$norm.method, "'. ", explNormalization)
}
...
diffbindSettings <- rbind(c(Parameter="DiffBind package version", Value=as.character(currentDiffbindVersion), Comment= paste(if(currentDiffbindVersion$major != floor(DIFFBINDVERSION)) {paste0("Major version not concordant with intended DiffBind v", DIFFBINDVERSION, ". Please check R module." )} else {""}, DiffBindWarningText)),
                          c("Apply auto-generated blacklist", BLACKLIST, if(BLACKLIST){if(is(db_all$parms$blacklist, "try-error")) {"Skipped because blacklist not available."} else {"Blacklist successfully generatedand applied."}} else {""}),
                          c("Apply auto-generated greylist", GREYLIST, if(GREYLIST){if(is(db_all$parms$greylist, "try-error")) {"Skipped because greylist not available."} else {"Greylist successfully generated and applied."}} else {""}),
                          ...
                          c("Subtract control read counts", SUBSTRACTCONTROL, if(SUBSTRACTCONTROL=="default") {paste("set to", db_all$parms$bSubControl, "because greylist is", if(db_all$parms$bSubControl){"not"} else {""}, "available.")}),
...
write.table(db_all$infodb, file=file.path(OUT, "info_dba_object.txt"), row.names = F, quote = F, sep="\t")
...
result <- lapply(result, function(x) {
  tryCatch({
    x[x$FDR<=db$config$th & abs(x$Fold)>=FOLD, ]
  }, error=function(e) x)
}) # filter result tables for significance
